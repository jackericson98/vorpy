...
cases = []
cases.append([Atom([0.5, -np.sqrt(3)*np.pi/8, 0], 0.25), Atom([0.5, np.sqrt(3)*np.pi/8, 0], 0.5), Atom([-0.5, 0, 0], 0.5)])
...
cases.append([Atom([0.5, -np.sqrt(3)*np.pi/16, 0], 0.5), Atom([0.5, np.sqrt(3)*np.pi/16, 0], 0.5), Atom([-0.5, 0, 0], 0.5)])
...
cases.append([Atom([0.25, -np.sqrt(3)*np.pi/10, 0], .25), Atom([0.25, np.sqrt(3)*np.pi/10, 0], .5), Atom([-.25, 0, 0], .5)])
...
for case_atoms in cases:
    ...
    myEdge.calc_points(surf=Surface(case_atoms[:2]), min_dist=.1)
...
for i in range(len(cases)):
    ...
    plot_verts(verts[i], fig=fig, ax=axn)
    plot_atoms(cases[i], fig=fig, ax=axn, colors=['r', 'b'], alpha=.1, dfo=2)
    plot_edges([edges[i]], fig=fig, ax=axn, dfo=5)
