...
cases.append([Atom([0.5, -0.5, 0], 0.5), Atom([0.5, 0.5, 0], 0.5), Atom([-0.5, 0, 0], 0.5)])
...
cases.append([Atom([0.5, -1, 0], 1), Atom([0.5, 0.5, 0], 0.75), Atom([-0.5, 0, 0], 0.5)])
...
for case_atoms in cases:
    ...
    myEdge.calc_points(min_dist=.05)
...
for i in range(len(cases)):
    ...
    plot_atoms(cases[i], fig=fig, ax=axn, colors=['b', 'b', 'b'], alpha=.1, dfo=3, grid=True, res=2)
    plot_edges([edges[i]], fig=fig, ax=axn, dfo=3, grid=False)
