...
def export_buried_water_groups(iface):
    ...
    Buried-water geometry is intentionally a subordinate interface product.
    Emit the files needed to identify, inspect, and visualize each closed water
    group: class-level PDBs, aggregate info, per-group info/PDB, the external
    shell surfaces/edges/vertices, and the standard Group AW log table.
    """
    ...
    for group_index, group in enumerate(groups, start=1):
        ...
        # Compact buried-water product: detailed Group info, standard AW logs,
        # and the external cell shell.  Full surfs/edges/verts remain omitted
        # because this is a subordinate object of the parent Interface.
        group.exports(
            info=True,
            shell_surfs=True,
            shell_edges=True,
