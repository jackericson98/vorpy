...
class ExportFrame(ttk.LabelFrame):
    ...
    def _create_widgets(self):
        ...
        # Numeric precision and geometry-file format
        ttk.Label(self, text="Round To:").grid(
            row=3, column=0, padx=2, pady=5, sticky="w"
        )
        ...
        round_spin.grid(row=3, column=1, sticky="w", padx=2, pady=5)

        round_spin.bind("<FocusOut>", lambda e: self._on_round_to_change())
        round_spin.bind("<Return>", lambda e: self._on_round_to_change())

        ttk.Label(self, text="File Format:").grid(
            row=3, column=2, padx=(12, 2), pady=5, sticky="e"
        )

        self.file_type = tk.StringVar(value=self.settings["file_type"].upper())
        ...
        self.file_type_combo.grid(
            row=3, column=3, columnspan=2, sticky="w", padx=2, pady=5
        )
        self.file_type_combo.bind("<<ComboboxSelected>>", self._on_file_type_change)
    ...
    def _on_file_type_change(self, *_):
        """Store the selected mesh format using its canonical lowercase name."""
        value = self.file_type.get().strip().lower()
        if value not in {"off", "ply", "vtp"}:
            value = "off"
            self.file_type.set(value.upper())
        self.settings["file_type"] = value
    ...
    def copy_settings_from(self, other_frame):
        ...
        self.file_type.set(self.settings.get("file_type", "off").upper())
        self._on_file_type_change()
