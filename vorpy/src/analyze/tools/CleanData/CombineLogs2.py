...
SECTION_TITLES = {
    ...
    'build informaiton': 'build information',  # legacy typo
...
def read_logs_sections(path):
    """Read a VorPy logs CSV into structured sections.

    Returns
    -------
    sections : OrderedDict[str, dict]
        keys are canonical section titles. Each value is:
        {
            "title": <section title>,
            "header": <header row list or None>,
    ...
    with open(path, 'r', newline='') as f:
        ...
        for row in rf:
            ...
            if first in SECTION_TITLES:
                title = SECTION_TITLES[first]
                if title not in sections:
                    sections[title] = {"title": title, "header": None, "rows": []}
                current = sections[title]
                expect_header = True
                continue

            if current is None:
                # Pre-header noise; ignore
                continue
...
def write_logs_sections(path, sections, section_order=None, renumber=True):
    """Write sections back to a logs CSV.
    ...
    path : str
        Output CSV path.
    sections : mapping
        Output of read_logs_sections (or compatible).
    section_order : list[str] | None
        Ordering of sections. Default uses build -> group -> Atoms -> Surfaces -> Edges -> Vertices.
    renumber : bool
        If True, renumber the first column (Index) for Atoms/Surfaces/Edges/Vertices to be 0..N-1.
        (Build/group sections are never renumbered.)
    """
    if section_order is None:
        section_order = ['build information', 'group information', 'Atoms', 'Surfaces', 'Edges', 'Vertices']
    ...
    with open(path, 'w', newline='') as out_f:
        ...
        for title in section_order:
            ...
            if sec.get("header") is not None:
                w.writerow(sec["header"])

            rows = sec.get("rows", [])
            if renumber and title in {'Atoms', 'Surfaces', 'Edges', 'Vertices'}:
                for i, r in enumerate(rows):
                    if not r:
                        continue
                    # If the row is already indexed, replace; else prepend.
                    if len(r) > 0 and re.fullmatch(r'-?\d+', str(r[0]).strip() or ''):
                        r2 = [i] + r[1:]
                    else:
                        r2 = [i] + r
                    w.writerow(r2)
