...
# Scatter/correlation plots to generate after the correlation matrix.
#
# Add/remove pairs here. Each tuple is:
#     ("X-axis metric", "Y-axis metric")
#
PLOT_PAIRS = [
    # ---------------------------------------------------------
    # Size / basic geometry
    ...
    # ---------------------------------------------------------
    # Size vs integrated mean curvature
    # ---------------------------------------------------------
    (
        "Radius",
    ...
    # ---------------------------------------------------------
    # Local mean vs Gaussian curvature
    # ---------------------------------------------------------
    (
    ...
    # ---------------------------------------------------------
    # Integrated curvature relationships
    # ---------------------------------------------------------
    (
        "Integrated Mean Curvature",
        "Integrated Mean Curvature Squared",
...
def main():
    ...
    # ---------------------------------------------------------------------
    # Generate all configured scatter/correlation plots
    # ---------------------------------------------------------------------

    pair_statistics = []

    print("\n" + "=" * 80)
    print("GENERATING SELECTED CORRELATION PLOTS")
    print("=" * 80)

    for plot_number, (x_metric, y_metric) in enumerate(PLOT_PAIRS, start=1):

        print()
        print(
            f"[{plot_number}/{len(PLOT_PAIRS)}] "
        ...
        if missing:
            print(
                "Skipping plot; unavailable metric(s): "
                + ", ".join(missing)
            )
            continue
        ...
        if np.isfinite(stats["Pearson r"]):
    ...
    # Save all pair statistics together.
    if pair_statistics:

        stats_path = (
            output_dir
            / "atomic_selected_pair_statistics.csv"
        )

        pd.DataFrame(pair_statistics).to_csv(
            stats_path,
            index=False,
        )

        print()
        print(f"Saved combined pair statistics: {stats_path}")

    else:
        print()
        print("No configured correlation plots could be generated.")
