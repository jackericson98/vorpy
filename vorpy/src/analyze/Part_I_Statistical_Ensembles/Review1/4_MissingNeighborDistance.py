...
def plot_data(ball_data):
    ...
    plt.plot(x_line, np.polyval(z_pw, x_line), 'b--', label='PW exclusive Fit', alpha=0.5)
    plt.plot(x_line, np.polyval(z_aw, x_line), 'r--', label='AW exclusive Fit', alpha=0.5)
    plt.plot(x_line, np.polyval(z_shared_pw, x_line), 'b-', label='Shared PW Fit', alpha=0.5)
    plt.plot(x_line, np.polyval(z_shared_aw, x_line), 'r-', label='Shared AW Fit', alpha=0.5)
    # colors = [np.log(abs(ball['vol_diff'])) for ball in data if all(key in ball for key in ['radius', 'aw_avg_neighbor_radius', 'vol_diff'])][::-1]

    plt.scatter(x, pw_avg_nbor_dist, c='blue', marker='x', label='PW only Neighbor Dist', alpha=0.3, s=15)
    plt.scatter(x, aw_avg_nbor_dist, c='red', marker='x', label='AW only Neighbor Dist', alpha=0.3, s=15)
    plt.scatter(x, shared_avg_nbor_pw_dist, c='blue', marker='o', label='Shared Avg Neighbor PW Dist', alpha=0.3, s=15)
    plt.scatter(x, shared_avg_nbor_aw_dist, c='red', marker='o', label='Shared Avg Neighbor AW Dist', alpha=0.3, s=15)
    ...
    plt.ylabel('Average Neighbor Distance', fontsize=20)
    plt.xticks(fontsize=16)
    plt.yticks(fontsize=16)
    plt.title('Average Neighbor Distance', fontsize=30)
