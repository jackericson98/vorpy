...
def _as_valid_edge_point(point, label="edge point"):
    arr = np.array(point, dtype=float)

    if arr.shape != (3,):
        raise ValueError(
            f"{label} is not a valid 3D point.\n"
            f"value = {point}\n"
...
def mid_edge_point(ep1, ep2, func, vmid, direction, new_direction=True, print_something=False,
                   edge_debug=None):
    """
    Calculates a middle point on an edge based on provided edge points, function, and direction.
    """
    ep1 = np.array(ep1, dtype=float)
    ep2 = np.array(ep2, dtype=float)
    ...
    if new_direction:
        ...
        if edist == 0 or not np.isfinite(edist):
            raise ValueError(
                "Cannot calculate edge midpoint because edge segment has zero/nonfinite length.\n"
                f"ep1={ep1}\n"
                f"ep2={ep2}\n"
                f"edge_debug={edge_debug}"
            )
        ...
        if rn_norm < 1e-12:
            ...
            if direction_norm < 1e-12:
                raise ValueError(
                    "Cannot calculate edge midpoint because proj_point == vmid "
                    "and fallback direction is zero."
                )
        ...
        if print_something or rn_norm == 0 or not np.isfinite(rn_norm):
            print("\n=== MID EDGE POINT DEBUG ===")
            print(f"ep1 = {ep1}")
            print(f"ep2 = {ep2}")
            print(f"edist = {edist}")
            print(f"proj_point = {proj_point}")
            print(f"vmid = {vmid}")
            print(f"rn = {rn}")
            print(f"rn_norm = {rn_norm}")
            print(f"incoming direction = {direction}")
            print(f"incoming direction norm = {np.linalg.norm(direction)}")
            print(f"edge_debug = {edge_debug}")
            print("============================\n")

        if rn_norm == 0 or not np.isfinite(rn_norm):
            fallback_norm = np.linalg.norm(direction)

            if fallback_norm == 0 or not np.isfinite(fallback_norm):
                raise ValueError(
                    "Cannot calculate edge midpoint because both recalculated direction "
                    "and fallback direction are zero/nonfinite.\n"
                    f"ep1={ep1}\n"
                    f"ep2={ep2}\n"
                    f"proj_point={proj_point}\n"
                    f"vmid={vmid}\n"
                    f"rn={rn}\n"
            ...
            direction = direction / fallback_norm
            ...
            if calc_dist(ep1, ep2) > res:
                try:
                    ...
                    edge_debug = {
                        ...
                        "res": res,
                        "edub": edub,
                        "num_existing_points": len(e_points),
                    }
                    ...
                    print(f"eballs = {eballs}")
                    print(f"vloc1 = {vlocs[0]}")
                    print(f"vloc2 = {vlocs[1]}")
                    print(f"vmid = {vmid}")
                    print(f"dnorm = {dnorm}")
                    print(f"edub = {edub}")
                    print(f"res = {res}")
                    print(f"edge points so far = {len(e_points)}")
                    print("==========================\n")
                    raise
