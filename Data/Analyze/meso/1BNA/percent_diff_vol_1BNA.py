...
if __name__ == '__main__':
    ...
    for file in vols:
        ...
        for res_name in vols[file]['nucs']:
            for i in range(len(vols[file]['nucs'][res_name])):
                my_perc_diff.append(
                    abs(float(vols['1BNA']['nucs'][res_name][i]) - float(vols[file]['nucs'][res_name][i])) / float(
                        vols['1BNA']['nucs'][res_name][i]))
    ...
    plt.title('1BNA Average Residue Variance (Volume)', fontdict=dict(size=20))
