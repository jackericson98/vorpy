...
if __name__ == '__main__':
    ...
    for file in sas:
        ...
        for res_name in vols[file]['amines']:
            for i in range(len(vols[file]['amines'][res_name])):
                my_perc_diff.append(abs(float(vols['181L']['amines'][res_name][i]) - float(
                    vols[file]['amines'][res_name][i])) / float(vols['181L']['amines'][res_name][i]))
    ...
    plt.title('181L Average Residue Variance (SA)', fontdict=dict(size=20))
