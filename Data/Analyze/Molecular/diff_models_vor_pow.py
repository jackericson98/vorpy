...
def percent_diff(files_dir, sa_vol='vol', martini=False):
    """A directory with all of the log files for x # of models is selected and the logs for pow and vor are compared"""
    pdb_files, log_files, names = [], []
    for root, dir, files in os.walk(files_dir):
        for file in files:
            if file[-3:] == 'pdb':
                pdb_files.append(files_dir + '/' + file)
                names.append(file[:-3])
            elif file[-3:] == 'csv':
                log_files.append(files_dir + '/' + file)
    for i, name in enumerate(names):
        # Check to see if the data has been processed yet
        if not os.path.exists(files_dir + name + '_residue_data.csv'):
            ...
            with open(files_dir + name + '_residue_data.csv', 'w') as res_file:
    ...
    for name in names:
        vols, sas = {}, {}
        with open(files_dir + name + '_residue_data.csv', 'r') as res_file:
