...
if plot_type == 'lognormal':
    my_densities = np.arange(0.05, 0.525, 0.025)
...
for i, num in enumerate(my_sds):
    for j, num2 in enumerate(my_densities):
        means, sds = [], []
        try:
            for lst in lists[num][num2]:
                means.append(np.mean(lst) * 100)
                sds.append(np.std([_ * 100 for _ in lst]) / np.sqrt(len(lst)))
        except KeyError:
            means, sds = [np.nan]*4, [np.nan]*4
