...
for folder in folders:
    ...
    print(f"Density: {density} --- Mean = {mean}, Absolute Mean = {abs_mean}")
    plt.plot([min(rads), max(rads)], [mean, mean], linestyle=':', c=color, linewidth=3)
    plt.plot([min(rads), max(rads)], [abs_mean, abs_mean], linestyle='--', c=color, linewidth=3)
    ...
    plt.plot(rads, [100 * _ for _ in func(np.array(rads), *popt)], c=color, linewidth=3, label=density)
