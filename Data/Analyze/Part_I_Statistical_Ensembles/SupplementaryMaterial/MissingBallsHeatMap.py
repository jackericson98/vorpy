...
def plot_heatmap(missing_balls, data=None, cv_values=None, density_values=None):
    ...
    ax.set_xlabel("CV Values", fontsize=14)
    ax.set_ylabel("Density Values", fontsize=14)
    ...
    ax.set_xticklabels([f"{v:.2f}" for v in cv_values])
    ax.set_yticklabels([f"{v:.2f}" for v in density_values])
    ...
    for i in range(len(density_values)):
        for j in range(len(cv_values)):
            if data[i, j] == 0:
                ...
                ax.text(j, i, f"{data[i, j]:.1f}", ha="center", va="center", color="black", fontsize=10)
