import os
from System.sys_funcs.input.surfs import read_surf_file
...
def build_surfs(grp, resolution=None):
    ...
    if resolution is not None:
        ...
        grp.surf_res = grp.net.settings['surf_res']
    # Go through the list of build surfaces checking for
    for i in grp.surfs:
        surf = grp.net.surfs.iloc[i]
        ...
        if surf['points'] is None or surf['tris'] is None or len(surf['points']) <= 2 or len(surf['tris']) == 0:
            ...
            try:
                grp.net.surfs.loc[i:i, ['points', 'tris', 'tri_curvs', 'curv', 'func', 'com', 'flat']] = spoints, surf_tris, tri_curvs, surf_curv, sfunc, surf_com, flat
            except ValueError:
                pass
    # Change back
    if grp.sys.files['dir'] is not None:
