...
class XTC:
    ...
    def allFrames(self, selection=None, steps=False, times=False, reverse=False):
        ...
        if steps or times:
            self.steps, self.times = zip(
                *[self.file.seek(i + 8) or unpack(">lf", self.file.read(8)) for i in self.frames])
...
if (options["-reverse"] or
    ...
    for xtc in xtcList:
        ...
        if options["-steps"]:
            print
            xtc.name, " ".join([str(i) for i in xtc.steps])
            continue

        if options["-times"]:
            print
            xtc.name, " ".join([str(i) for i in xtc.times])
            continue
