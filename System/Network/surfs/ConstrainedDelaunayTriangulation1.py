...
import numpy as np
from System.sys_funcs.calcs.calcs import calc_dist, calc_tri, calc_com
...
def sort_tris(perimeter, tris, polygon, points):
    ...
    for tri in tris:
        ...
        if all(tri_points):
            # Check that the triangle is inside the perimeter
            if polygon.contains(Point(calc_com([[points[_].x, points[_].y] for _ in tri]))):
                in_.append(tri)
