...
from System.Network.surfs.fill import fill_mesh, calc_surf_point, calc_surf_point_from_plane
from System.sys_funcs.calcs.calcs import calc_com, project_to_plane, calc_dist, unproject_to_3d
import numpy as np
from shapely.plotting import plot_polygon
from shapely import Polygon, Point
from System.Network.surfs.triangulate import triangulate_2D_Surface1, is_within, triangulate_2D_Surface
...
# Build method. Makes the mesh for the surface and calculates the simplices between them
def build_surf1(locs, rads, epnts, res, net_type, sfunc=None, check=False, timer=False, plotting=False):
    """
    Main build method for constructing surfaces
    :param surf:
    :param res: Specifies the resolution the surface is to be constructed with
    ...
    test_3d_points = unproject_to_3d(flat_points, plane_point=surf_loc, plane_normal=surf_norm)

    # fig = plt.figure()
    # ax = fig.add_subplot(projection='3d')
    # ax.scatter([_[0] for _ in perimeter], [_[1] for _ in perimeter], [_[2] for _ in perimeter])
    # ax.scatter([_[0] for _ in test_3d_points], [_[1] for _ in test_3d_points], [_[2] for _ in test_3d_points])
    # plt.show()
    ...
    if net_type == 'aw' and not flat and perim_poly.contains(Point(surf_loc)):
        surf_curv = calc_surf_point_curv(sfunc, surf_loc)
    # Filter out the bad triangles
    my_2d_points, surf_tris = triangulate_2D_Surface1(flat_points, res=res, center=flat_loc)
