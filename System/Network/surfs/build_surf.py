...
from System.sys_funcs.calcs.calcs import calc_com, rotate_points
from Visualize.mpl_visualize import plot_balls, plot_surfs
import numpy as np
...
def build_surf1(locs, rads, epnts, res, net_type, sfunc=None):
    ...
    nps = rotate_points(np.array(surf_norm), np.array(points))
    ...
    triangulated_points = rotate_points(-np.array(surf_norm), np.array([[*_, 0] for _ in triangulation['vertices']]),
                                        reverse=True)
    ...
    # Return the surface points, triangles, triangle curvatures, total curvature, surface function, com, and flatness
    return np.array(spoints), np.array(triangulation['triangles']), tri_curvs, surf_curv, sfunc, surf_com, flat
