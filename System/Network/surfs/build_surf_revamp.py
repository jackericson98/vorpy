...
from System.Network.surfs.ConstrainedDelaunayTriangulation import triangulate_2D_Surface
...
def build_surf_new(locs, rads, epnts, res=0.1, plotting=False, net_type='aw'):
    """
    1. Calc_surf_func
    2. Build Perimeter
    3. Calculate plane
    ...
    perimeter = build_perimeter1(epnts)

    print([_ for _ in perimeter])
    fig = plt.figure()
    ...
    for i in range(len(perimeter)):
        pep, plp = perimeter[i], plane_points[i]
        plt.plot([pep[0], plp[0]], [pep[1], plp[1]], [pep[2], plp[2]], linewidth=0.3)
        plt.plot([a0x, plp[0]], [a0y, plp[1]], [a0z, plp[2]], linewidth=0.3, linestyle='dashed')
    ...
    # plt.scatter([_[0] for _ in twoD_points], [_[1] for _ in twoD_points])
    # plt.show()
    # Step 6: Fill in with Delaunay triangulation
    spoints, stris = triangulate_2D_Surface(twoD_points, res)

    # Step 7: Rotate and translate back to the plane
    new_plane_points = reverse_transform_points(spoints, rot_mtx, ctr_pnt)
    ...
    # ax.scatter([_[0] for _ in new_plane_points], [_[1] for _ in new_plane_points], [_[2] for _ in new_plane_points],
    #            marker='x')
    # # # Plot the rotated points
    # # ax.scatter([_[0] for _ in surface_points], [_[1] for _ in surface_points], [_[2] for _ in surface_points],
    # #            marker='x')
    ...
    if not flat:
        ...
        surface_points = array(new_plane_points)
    ...
    if plotting:
        ...
        # Show everything
        plt.show()
    # plot_surfs([surface_points], [stris], simps=True, Show=True, alpha=1, colors=['k'])

    # Calculate the curvature of the triangles and the surface
    if not flat:
        surf_curv = calc_surf_point_curv(sfunc, p_point)
        tri_curvs, surf_curv = calc_surf_tri_curvs(sfunc, surface_points, stris, max_curv=surf_curv)
    else:
        tri_curvs, surf_curv = [0 for _ in range(len(list(stris)))], 0
    return surface_points, stris, tri_curvs, surf_curv, sfunc, p_point, flat
